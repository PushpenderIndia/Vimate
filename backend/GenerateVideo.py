...
from GenerateVoiceV2 import GenerateVideoV2
...
class GenerateVideo:
    def __init__(self, topic, ELEVEN_LABS_API, language="en"):
        ...
        self.voicegen1 = GenerateVideoV2()
        self.voicegen2 = GenerateVoice(ELEVEN_LABS_API)
    ...
    def generate_audio(self, text, filename, language="en"):
        try:
            self.voicegen1.convert_text_to_speech(text, filename)
            print(f"{filename} generated using ElevenLabs")
        except:
            try:
                self.voicegen2.convert_text_to_speech(text, filename)
                print(f"{filename} generated using ElevenLabs")
            except: 
